...
export const AuthScreen: React.FC<AuthScreenProps> = ({ recoveryMode = false, passwordSetupMode = false, onPasswordSet }) => {
  ...
  useEffect(() => { if (isPasswordSetup) setMode('update_password'); }, [isPasswordSetup]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!supabase) return;
    setError(''); setSuccess('');
    if (mode === 'update_password' && password.length < 6) {
      setError('A senha deve ter pelo menos 6 caracteres.'); return;
    }
    if (mode === 'update_password' && password !== confirmPassword) {
  ...
  const titles: Record<AuthMode, string> = { sign_in: 'Acesse sua conta', forgot_password: 'Recupere sua senha', update_password: passwordSetupMode ? 'Defina sua senha' : 'Defina uma nova senha' };
  const descriptions: Record<AuthMode, string> = { sign_in: 'Entre para gerenciar as escalas da sua farmácia.', forgot_password: 'Informe seu e-mail e enviaremos um link seguro.', update_password: passwordSetupMode ? 'Seu acesso foi criado. Escolha uma senha para continuar.' : 'Escolha uma senha nova para continuar.' };
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-8 flex items-center justify-center">
    <section className="w-full max-w-md overflow-hidden rounded-3xl border border-slate-700 bg-white shadow-2xl shadow-slate-950/40">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 p-7">
        ...
        <button disabled={isSubmitting} className="flex w-full items-center justify-center gap-2 rounded-xl bg-sky-600 px-4 py-3 text-sm font-bold text-white transition hover:bg-sky-500 disabled:cursor-wait disabled:opacity-60">{mode === 'forgot_password' && <KeyRound className="h-4 w-4" />}{isSubmitting ? 'Aguarde...' : mode === 'sign_in' ? 'Entrar' : mode === 'forgot_password' ? 'Enviar link' : 'Salvar senha e continuar'}</button>
        <div className="space-y-2 text-center text-sm">
          {mode === 'sign_in' && <button type="button" onClick={() => changeMode('forgot_password')} className="block w-full text-sky-700 hover:underline">Esqueci minha senha</button>}
          {mode === 'forgot_password' && <button type="button" onClick={() => changeMode('sign_in')} className="inline-flex items-center gap-1 text-slate-600 hover:text-sky-700 hover:underline"><ArrowLeft className="h-3.5 w-3.5" />Voltar para entrar</button>}
        </div>
      </form>
    </section>
  </main>;
