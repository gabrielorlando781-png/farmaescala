...
export const StoreManagerPanel: React.FC<StoreManagerPanelProps> = ({ organization, canCreateStores, onClose }) => {
  ...
  useEffect(() => {
    if (!managerStoreId && stores[0]) setManagerStoreId(stores[0].id);
  ...
  const inviteManager = async (event: FormEvent) => {
    ...
    try {
      ...
      setSuccess(`Convite de gerente enviado para ${managerEmail}.`);
  ...
  return <main className="min-h-screen bg-slate-100 p-4 sm:p-8"><div className="mx-auto max-w-5xl">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_1.1fr]">
      <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200"><h2 className="text-lg font-bold text-slate-900">Cadastrar filial</h2>{canCreateStores ? <form onSubmit={submit} className="mt-5 space-y-4"><label className="block text-sm font-semibold text-slate-700">Nome da filial<input required value={name} onChange={(e) => setName(e.target.value)} placeholder="Unidade Centro" className="mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100" /></label><label className="block text-sm font-semibold text-slate-700">Código interno<input required value={code} onChange={(e) => setCode(e.target.value)} placeholder="CENTRO-01" className="mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 uppercase outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100" /></label><div className="grid grid-cols-[1fr_72px] gap-3"><label className="block text-sm font-semibold text-slate-700">Cidade<input value={city} onChange={(e) => setCity(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:border-sky-500" /></label><label className="block text-sm font-semibold text-slate-700">UF<input maxLength={2} value={state} onChange={(e) => setState(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 uppercase outline-none focus:border-sky-500" /></label></div>{error && <p className="rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-700">{error}</p>}{success && <p className="rounded-xl bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{success}</p>}<button disabled={saving} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-sky-600 px-4 py-3 text-sm font-bold text-white hover:bg-sky-500 disabled:opacity-60"><Plus className="h-4 w-4" />{saving ? 'Cadastrando...' : 'Cadastrar filial'}</button></form> : <div className="mt-5 rounded-2xl bg-amber-50 p-4 text-sm leading-6 text-amber-800"><LockKeyhole className="mb-2 h-5 w-5" />A criação de filiais está bloqueada para esta rede. Solicite a liberação ao administrador da plataforma.</div>}{stores.length > 0 && <><div className="my-6 border-t border-slate-200" /><h2 className="flex items-center gap-2 text-lg font-bold text-slate-900"><UserPlus className="h-5 w-5 text-sky-600" />Convidar gerente de filial</h2><form onSubmit={inviteManager} className="mt-5 space-y-4"><label className="block text-sm font-semibold text-slate-700">Filial<select required value={managerStoreId} onChange={(e) => setManagerStoreId(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:border-sky-500">{stores.map((store) => <option key={store.id} value={store.id}>{store.name} · {store.code}</option>)}</select></label><label className="block text-sm font-semibold text-slate-700">Nome do gerente<input required value={managerName} onChange={(e) => setManagerName(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:border-sky-500" /></label><label className="block text-sm font-semibold text-slate-700">E-mail do gerente<span className="relative mt-1.5 block"><Mail className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-slate-400" /><input required type="email" value={managerEmail} onChange={(e) => setManagerEmail(e.target.value)} className="w-full rounded-xl border border-slate-300 py-2.5 pl-10 pr-3 outline-none focus:border-sky-500" /></span></label><button disabled={inviting} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-slate-800 px-4 py-3 text-sm font-bold text-white hover:bg-slate-700 disabled:opacity-60"><UserPlus className="h-4 w-4" />{inviting ? 'Enviando...' : 'Convidar gerente'}</button></form></>}</section>
