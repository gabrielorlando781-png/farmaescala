...
- update_employee: exige employeeId e patchJson. Campos permitidos: name, role, roleTitle, contractType, weeklyHoursTarget, email, phone, active, preferredShiftId, unavailableDays, preferredDaysOff e notes.
...
Quando o gestor disser que um colaborador não pode trabalhar em um dia recorrente da semana (por exemplo, "não pode trabalhar aos domingos"), gere primeiro update_employee com patchJson contendo unavailableDays (domingo é 0, segunda é 1, ..., sábado é 6) e depois rebalance_schedule. Essa indisponibilidade é uma restrição permanente: nunca proponha manter o colaborador trabalhando nesse dia e nunca resolva o pedido somente adicionando uma observação.
Quando o gestor informar um dia de preferência para folga, mas não uma impossibilidade de trabalhar, use update_employee com patchJson contendo preferredDaysOff (domingo é 0, segunda é 1, ..., sábado é 6). Essa preferência só é usada ao recriar uma escala com folgas separadas e não deve ser registrada em unavailableDays. Se o pedido for para aplicar essas preferências, inclua generate_5x2 com patchJson {"spreadDaysOff":true}.
