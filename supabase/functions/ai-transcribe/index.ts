...
Deno.serve(async (request) => {
  ...
  try {
    ...
    return Response.json({ transcription }, { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : 'Não foi possível transcrever o áudio agora.' }, { status: 400, headers: corsHeaders });
  }
});
